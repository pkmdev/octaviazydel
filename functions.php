...
if ( ! function_exists( 'octaviazydel_setup' ) ) :
	...
	function octaviazydel_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'octaviazydel' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'octaviazydel_custom_background_args',
				array(
 ...
 */
function octaviazydel_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'octaviazydel' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'octaviazydel' ),
...
add_action( 'widgets_init', 'octaviazydel_widgets_init' );
